...
async function setupArray(message) {
    ...
    await channel.fetch({ limit: 20 }).then(messages => {
        ...
        .catch(console.error);
    lastMessages.unshift({ role: "system", content: fs.readFileSync("./src/supportPrompt.txt", "utf-8").replace("<discordid>", message.author.id) });
}
client.on("messageCreate", async function (message) {
    if (message.author.bot) return;
    if (message.channelId == "1142418048607854714") {
